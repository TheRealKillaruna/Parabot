...
// free all memory allocated for botchat
{
	...
	for (unsigned int r=0; r<chatReplies.size(); r++ ) {
...
// analyze the message the speaker says
{
	...
	// find keyword
	char *wordFound = 0;
	unsigned int replyIdx;
	for (replyIdx=0; replyIdx<chatReplies.size(); replyIdx++) {
		wordFound = strstr( parseBuffer, chatReplies[replyIdx]->code );
		if (wordFound) break;
	}
	...
	if (wordFound) answer = getMessageFromList( *(chatReplies[replyIdx]->reply), forceAnswer );
