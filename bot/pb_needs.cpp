...
void PB_Needs::valveWishList()
{
	...
	for (int i=0; i<MAX_NAV_TYPES; i++) 
		if ( mapGraph.itemAvailable(i) && (wish[i]>maxWish) )
			maxWish = wish[i];
	weaponWish = 0;
	for (int i=NAV_W_CROSSBOW; i<=NAV_A_GLOCKCLIP; i++) 
...
void PB_Needs::hwWishList()
{
	...
	weaponWish = 0;
	for (int i=NAV_HWW_DOUBLESHOTGUN; i<=NAV_HWA_ROCKETLAUNCHER; i++) 
		if ( mapGraph.itemAvailable(i) ) weaponWish += wish[i];
}
...
void PB_Needs::dmcWishList()
{
	...
	for (int i=0; i<MAX_NAV_TYPES; i++) 
		if ( mapGraph.itemAvailable(i) && (wish[i]>maxWish) ) maxWish = wish[i];
	weaponWish = 0;
	for (int i=NAV_DMCW_QUAKEGUN; i<=NAV_DMCW_LIGHTNING; i++) 
		if ( mapGraph.itemAvailable(i) ) weaponWish += wish[i];
}
...
void PB_Needs::gearboxWishList()
{
	...
	for (int i=NAV_W_CROSSBOW; i<=NAV_A_GLOCKCLIP; i++) 
		if ( mapGraph.itemAvailable(i) ) weaponWish += wish[i];
	for (int i=NAV_OFW_GRAPPLE; i<=NAV_OFA_SPORE; i++) 
		if ( mapGraph.itemAvailable(i) ) weaponWish += wish[i];
